...
def finish_game(score, window):
    ...
    s = f'Você perdeu! Conseguiu o total de: {score} frutas!'
    y = int(height / 2)
    x = int((width - len(s)) / 2)
    window.addstr(y, x, s)
    window.refresh()
...
def select_dificulty():
    ...
    while True:
        answer = input('Selecione uma das dificudades de 1 a 5:')
        game_speed = difficulty.get(answer)
        if game_speed is not None:
            return game_speed
        print('Escolha a dificuldade de 1 a')

if __name__ == '__main__':
    curses.wrapper(game_loop, game_speed=select_dificulty())
